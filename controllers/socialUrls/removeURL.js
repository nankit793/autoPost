...
const deleteURL = async (url, URLmodel, name) => {
    try {
        ...
        if (!element.uploadedToFb && !element.uploadedToInstagram && !element.uploadedToYoutube) {
            ...
            return { state: false, message: "seems like url is uploaded in at least one social media, contact developer" }
        }
    }
    catch (error) {
        return { state: false, message: error.message || "Server error, Please contact developer" }
    }
}
