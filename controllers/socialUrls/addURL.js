...
const addURL = async (mediaLinks, name, URLmodel) => {
    ...
    try {
        ...
        if (processReq && processReq.state && processReq.newUrl) {
            return { state: true, message: 'URL added successfully', doc: processReq.newUrl }
            //   return res.status(200).json({ message: 'URL added successfully', doc: processReq.newUrl });
        }
        else {
            return { state: false, message: processReq.message || "server error, contact developer" }
