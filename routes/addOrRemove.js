...
app.post('/addURL', async (req, res) => {
    try {
        ...
        if (add.state)
            return res.status(200).json({ message: add.message || "Success", doc: add?.doc || {} })
        else
            return res.status(401).json({ message: add.message || "error" })
...
app.delete('/clearURL', async (req, res) => {
    try {
        ...
        if (deleter.state)
            ...
            return res.status(401).json({ message: deleter.message || "error" })
    } catch (error) {
...
app.get("/info", async (req, res) => {
    try {
        ...
        if (info.state)
            return res.status(200).json({ message: info.message || "Success", ...info })
        else
            return res.status(401).json({ message: info.message || "error" })
